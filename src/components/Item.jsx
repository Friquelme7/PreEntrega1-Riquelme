  ...
  return (
    <div className="font-Montserrat max-w-xs mx-auto my-5 rounded-md shadow-xl bg-white p-3 cursor-pointer hover:border-[2px] transition hover:transition-opacity duration-200 delay-200 ease-linear border-[#403D39]">
      <Link to={`/products/${product.id}`}>
        <img
          src={product.img}
          alt={`imagen de un ${product.type} titulo ${product.productName}`}
        />
        <h2 className="font-alata font-medium mt-2 text-xl">
        ...
        <button className="flex justify-center font-Montserrat text-sm bg-[#D90429] hover:bg-[#EF233C] rounded-lg  border-black px-3 py-3 text-white font-medium">
          Agregar al carrito{" "}
          <svg
